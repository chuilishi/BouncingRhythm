...
{   
    ...
    public class RhythmToolWindow : EditorWindow
    {
        ...
        private void Saver()
        {
            if(state.curTrack == null)return;
            var chart = state.curTrack.chart;
        ...
        private void Toolbar()
        {
            using (new GUILayout.HorizontalScope(EditorStyles.toolbar, GUILayout.ExpandWidth(true)))
            {
                ...
                EditorGUILayout.IntField(BPM,GUILayout.Width(50));
                EditorGUILayout.LabelField("offset",GUILayout.Width(50));
                var tmp = EditorGUILayout.Slider(offset, -0.5f, 0.5f);
                if (tmp != offset)
                {
                    offset = tmp;
                    rhythmData.chart.offset = tmp;
                    rhythmData.chart.UpdateKeys(rhythmData.chart.SecondPerBeat,rhythmData.chart.offset);
                    var trackGUI = (TrackGUI<Beat>)tracks[0];
                    foreach (var featureGUI in trackGUI.features)
                    {
                        var feature = ((FeatureGUI<Beat>)featureGUI).feature;
                        feature.timestamp = feature.key.Time;
                    }
                    trackGUI.Refresh();
                }
                if (GUILayout.Button(Styles.saveButton, GUI.skin.button))
                {
                    var chart = state.curTrack.chart;
                    foreach (var track in chart.tracks)
                    {
        ...
        private void SetRhythmData(RhythmData rhythmData)
        {
            ...
            BPM = rhythmData.chart.BPM * rhythmData.chart.multiple;
            
            offset = rhythmData.chart.offset;

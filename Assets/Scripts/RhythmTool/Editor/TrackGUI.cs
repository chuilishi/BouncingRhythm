...
namespace RhythmTool
{
    ...
    public class TrackGUI<T> : TrackGUI where T : Feature, new()
    {
        ...
        public List<FeatureGUI> features;
        ...
        /// <summary>
        /// RhythmToolWindow的保存按钮会执行的函数
        /// </summary>
        public void SaveToChart()
        {
            // var keys = track.chart.keys = new List<IKey>();
            // for (int i = 0; i < features.Count; i++)
            // {
            //     var feature = ((FeatureGUI<Beat>)features[i]).feature;
            //     var key = feature.key;
            //     switch (feature.key.keyType)
            //     {
            //         case KeyType.LeftSingle or KeyType.LeftDouble or KeyType.RightSingle or KeyType.RightDouble:
            //             keys.Add(new Tap(feature.key.keyType,));
            //             break;
            //         case KeyType.LeftSimultaneous or KeyType.RightSimultaneous:
            //             
            //             break;
            //     }
            //     
        ...
        private void CreateFeature()
        {
            ...
            float timestamp = state.PixelsToTimestamp(state.mouseDownPosition.x);
            //FIXED
            var alignedTimeStamp = Aligner.Instance.Align(timestamp);

            float value = (rect.height - state.mouseDownPosition.y) / rect.height;
            //FIXED
            T newFeature = featureCreator.Create(alignedTimeStamp, value);
        ...
        private void DeleteFeatures()
        {
            ...
            {
                track.chart.keys.Remove(feature.key);
        ...
        private void PasteFeatures(float timestamp)
        {
            ...
            foreach (T feature in features)
            {
                ...
                track.chart.PushBackKey(copy.key);
                track.Add(copy);
